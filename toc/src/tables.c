...
str_table* create_str_table(str_table **last_string_table_node, str_table *e_string_table, translation_unit *tu, unsigned int *ret){
  *ret=1;
  ...
  while(phrase_statement_sequence!=NULL){
    ...
    if(phrase_statement->child_type==phrase_type_push_statement && phrase_statement->child.phrase_push_statement->child_type==lex_type_string_constant && lookup_in_string_table(phrase_statement->child.phrase_push_statement->child.string_constant,e_string_table)==-1){
      ...
      if((curr->string=str2cseq(phrase_statement->child.phrase_push_statement->child.string_constant))==NULL){
        fprintf(stderr,"implementation error: failed to allocate enough memory\n");
        return string_table;
      }
      curr->ptr=0;
    }
    else if(phrase_statement->child_type==phrase_type_set_statement && phrase_statement->child.phrase_set_statement->child1_type==lex_type_string_constant && lookup_in_string_table(phrase_statement->child.phrase_set_statement->child1.string_constant,e_string_table)==-1){
      if(string_table==NULL){
        if((string_table=malloc(sizeof(str_table)))==NULL){
          fprintf(stderr,"implementation error: failed to allocate enough memory\n");
          return NULL;
      ...
      if((curr->string=str2cseq(phrase_statement->child.phrase_set_statement->child1.string_constant))==NULL){
        fprintf(stderr,"implementation error: failed to allocate enough memory\n");
        return string_table;
      }
      curr->ptr=0;
...
nightVM_l lookup_in_symbol_table(char *s, sym_table *symbol_table, label **ret_label, set_statement **ret_set_statement, translation_unit *tu, bool stop_early){
  while(symbol_table!=NULL){
    ...
    if(strcmp(symbol_table->symbol_identifier,s)==0 && (symbol_table->tu==tu || symbol_table->visibility_code==visibility_exposed)){
      if(symbol_table->sym_type==sym_label){
        if(ret_label!=NULL){
          *ret_label=symbol_table->tu_loc.label_loc;
        }
        if(ret_set_statement!=NULL){
          *ret_set_statement=NULL;
        }
      }
      else if(symbol_table->sym_type==sym_nonlabel){
        if(ret_set_statement!=NULL){
          *ret_set_statement=symbol_table->tu_loc.set_statement_loc;
        }
        if(ret_label!=NULL){
          *ret_label=NULL;
        }
...
static inline void add_libraries_init_to_address(libs *libraries, nightVM_l *address){
  while(libraries!=NULL){
    if((*address+1)%ALIGNOF_L!=0){
      *address+=((*address+1)+ALIGNOF_L-1)-(((*address+1)+ALIGNOF_L-1)%ALIGNOF_L)-(*address+1);
    }
    if(comp_attr.pic){
  ...
  *ret=1;
  ...
  while(phrase_statement_sequence!=NULL){
    ...
    if(phrase_statement->child_type==phrase_type_label){
      ...
      lookup_in_symbol_table(phrase_statement->child.phrase_label->identifier,e_symbol_table,&ret_label,&ret_set_statement,tu,false);
      ...
      if(strcmp(phrase_statement->child.phrase_label->identifier,"main")==0){
        added_main=true;
        ...
        curr->symbol_identifier=phrase_statement->child.phrase_label->identifier;
        curr->val=*address;
      }
      ...
      curr->tu=tu;
      curr->symbol_identifier=phrase_statement->child.phrase_label->identifier;
      curr->tu_loc.label_loc=phrase_statement->child.phrase_label;
      curr->sym_type=sym_label;
      curr->val_filled=true;
    }
    else if(phrase_statement->child_type==phrase_type_set_statement){
      label *ret_label;
      set_statement *ret_set_statement;
      lookup_in_symbol_table(phrase_statement->child.phrase_set_statement->identifier,e_symbol_table,&ret_label,&ret_set_statement,tu,false);
      if(ret_label!=NULL || ret_set_statement!=NULL){
      ...
      if(phrase_statement->child.phrase_set_statement->child1_type==lex_type_numeric_constant){
        curr->val=numeric_constant_to_l(phrase_statement->child.phrase_set_statement->child1.numeric_constant);
        ...
        curr->val=phrase_statement->child.phrase_set_statement->child1.character_constant[0];
        if(phrase_statement->child.phrase_set_statement->phrase_sign!=NULL){
          curr->val*=phrase_statement->child.phrase_set_statement->phrase_sign->sign_code;
        }
        curr->val_filled=true;
        ...
        if(added_main){
          curr->val=lookup_in_string_table(phrase_statement->child.phrase_set_statement->child1.string_constant,string_table);
          if(phrase_statement->child.phrase_set_statement->phrase_sign!=NULL){
            curr->val*=phrase_statement->child.phrase_set_statement->phrase_sign->sign_code;
          }
          curr->val_filled=true;
          ...
          curr->val_filled=false;
        }
      }
      else if(phrase_statement->child.phrase_set_statement->child1_type==phrase_type_symbol){
        label *ret_label;
        set_statement *ret_set_statement;
        curr->val=lookup_in_symbol_table(phrase_statement->child.phrase_set_statement->child1.phrase_symbol->identifier,e_symbol_table,&ret_label,&ret_set_statement,tu,true);
        if(phrase_statement->child.phrase_set_statement->phrase_sign!=NULL){
          curr->val*=phrase_statement->child.phrase_set_statement->phrase_sign->sign_code;
        }
        if(ret_label==NULL && ret_set_statement==NULL){
      ...
      curr->tu=tu;
      curr->symbol_identifier=phrase_statement->child.phrase_set_statement->identifier;
      curr->tu_loc.set_statement_loc=phrase_statement->child.phrase_set_statement;
      curr->sym_type=sym_nonlabel;
    }
      ...
      if(phrase_statement->child.phrase_push_statement->phrase_push_instruction->push_instruction_code==key_pushuc){
        *address+=1+1;
        ...
        if((*address+1)%ALIGNOF_US!=0){
          *address+=((*address+1)+ALIGNOF_US-1)-(((*address+1)+ALIGNOF_US-1)%ALIGNOF_US)-(*address+1);
        }
        *address+=1+SIZEOF_US;
      }
        ...
        if((*address+1)%ALIGNOF_UI!=0){
          *address+=((*address+1)+ALIGNOF_UI-1)-(((*address+1)+ALIGNOF_UI-1)%ALIGNOF_UI)-(*address+1);
        }
        *address+=1+SIZEOF_UI;
      }
        ...
        *address+=1+1;
        ...
        if((*address+1)%ALIGNOF_S!=0){
          *address+=((*address+1)+ALIGNOF_S-1)-(((*address+1)+ALIGNOF_S-1)%ALIGNOF_S)-(*address+1);
        }
        *address+=1+(SIZEOF_S);
      }
        ...
        if((*address+1)%ALIGNOF_I!=0){
          *address+=((*address+1)+ALIGNOF_I-1)-(((*address+1)+ALIGNOF_I-1)%ALIGNOF_I)-(*address+1);
        }
        *address+=1+(SIZEOF_I);
      }
      ...
      *address+=1+(SIZEOF_L);
      ...
      if(phrase_statement->child.phrase_instruction->instruction_code==key_call){
        if((*address+2)%ALIGNOF_L!=0){
          *address+=((*address+2)+ALIGNOF_L-1)-(((*address+2)+ALIGNOF_L-1)%ALIGNOF_L)-(*address+2);
        }
  ...
  if(added_main){
    sym_table *trav_symt=e_symbol_table;
    while(trav_symt!=NULL){
      if(trav_symt->sym_type==sym_nonlabel && !trav_symt->val_filled){
        if(trav_symt->tu_loc.set_statement_loc->child1_type==lex_type_string_constant){
          trav_symt->val=lookup_in_string_table(trav_symt->tu_loc.set_statement_loc->child1.string_constant,string_table);
          if(trav_symt->tu_loc.set_statement_loc->phrase_sign!=NULL){
            trav_symt->val*=trav_symt->tu_loc.set_statement_loc->phrase_sign->sign_code;
          }
          trav_symt->val_filled=true;
        }
        else if(trav_symt->tu_loc.set_statement_loc->child1_type==phrase_type_symbol){
          label *ret_label;
          set_statement *ret_set_statement;
          trav_symt->val=lookup_in_symbol_table(trav_symt->tu_loc.set_statement_loc->child1.phrase_symbol->identifier,e_symbol_table,&ret_label,&ret_set_statement,tu,false);
          if(trav_symt->tu_loc.set_statement_loc->phrase_sign!=NULL){
          trav_symt->val*=trav_symt->tu_loc.set_statement_loc->phrase_sign->sign_code;
          }
          if(ret_label==NULL && ret_set_statement==NULL){
            write_error("error: use of unset symbol\n",trav_symt->tu_loc.set_statement_loc->start_x,trav_symt->tu_loc.set_statement_loc->start_y,trav_symt->tu_loc.set_statement_loc->end_x,trav_symt->tu_loc.set_statement_loc->end_y,trav_symt->tu_loc.set_statement_loc->file);
            return symbol_table;
          }
          trav_symt->val_filled=true;
        }
      }
      trav_symt=trav_symt->next_node;
    }
  }
  *ret=0;
  ...
  while(struct_table!=NULL){
    ...
    if(strcmp(struct_table->struct_name_identifier,s)==0 && struct_table->tu==tu){
      if(phrase_struct_definition!=NULL){
        *phrase_struct_definition=struct_table->loc;
      }
  ...
  *ret=1;
  ...
  while(phrase_struct_tag_definition_sequence!=NULL){
    ...
    if(phrase_struct_tag_definition->child_type==phrase_type_type){
      ...
      size+=lookup_in_struct_definition_table(phrase_struct_tag_definition->child.phrase_struct_name->identifier,struct_table,tu,true,&ret,ret_size);
      if(ret==NULL){
  ...
  *ret=1;
  ...
  while(phrase_statement_sequence!=NULL){
    ...
    if(phrase_statement->child_type==phrase_type_struct_definition){
      ...
      lookup_in_struct_definition_table(phrase_statement->child.phrase_struct_definition->identifier,e_struct_table,tu,false,&ret_sd,ret_any);
      if(ret_sd!=NULL){
      ...
      curr->struct_name_identifier=phrase_statement->child.phrase_struct_definition->identifier;
      curr->tu=tu;
      unsigned int ret;
      ...
      curr->loc=phrase_statement->child.phrase_struct_definition;
    }
    phrase_statement_sequence=phrase_statement_sequence->phrase_statement_sequence;
  }
  *ret=0;
